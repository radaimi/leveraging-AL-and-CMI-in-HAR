...
#    y = y.reshape((-1,1))
    ...
    pd.DataFrame(train_data).to_csv("./results/train_data_fold_" + str(fold) + "_sensor_" + str(sensors_to_use) + "_target_label_" + str(target_label) + "_AL.csv")
 
    file1 = open("./results/selected_indx_fold_" + str(fold) + "_sensor_" + str(sensors_to_use) + "_target_label_" + str(target_label) + "_AL.csv", "w")
    ...
    for b in range(n_batches):
        ...
        pd.DataFrame(m).to_csv("./results/min_margin_fold_" + str(fold) + "_sensor_" + str(sensors_to_use) + "_target_label_" + target_label + "_n_iter_" + str(b) + "_informative_diverse.csv")
...
Context Labels used:
    ...
    - Bathing - Shower
"""
# path = "./ExtraSensory.per_uuid_features_labels"

# participant_files = os.listdir(path)
#p = os.path.splitext(os.path.splitext(os.path.splitext(participant_files[0])[0])[0])[0]
...
def select_batch(sampler, N, already_selected,
    ...
    batch_AL, prob_margin = sampler.select_batch(**kwargs)
    already_selected = already_selected + batch_AL
    return batch_AL, prob_margin


results_per_participant = csv.writer(open("./results/results_AL_Informative_Diverse.csv", "w"))
