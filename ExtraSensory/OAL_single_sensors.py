# -*- coding: utf-8 -*-
"""
Created on Fri Aug 17 11:16:09 2018
...
def train_test_model(X_train,Y_train,M_train,feat_sensor_names,label_names,sensors_to_use,target_label,seed, X_test,y_test,M_test,timestamps_test,fold, results_per_participant):
...
#    y = y.reshape((-1,1))
    ...
    requested = True
    select_model = score_model = sklearn.linear_model.LogisticRegression(class_weight='balanced');
    started = True
    for b in range(n_samples):
        ...
        if (requested):
            ...
            print("Training loco model on " + str(n_train) + " datapoints")
            ...
            model = {\
                    ...
                    'mean_vec':mean_vec,\
                    'std_vec':std_vec,\
                    'lr_model':select_model};
            
            test_model(X_test,y_test,M_test,timestamps_test,feat_sensor_names,label_names,model,fold, n_train, results_per_participant);
...
#        n_sample = min(batch_size, train_size - len(selected_inds))
        ...
        if started == False:
            started = True
            requested = True
            selected_inds.extend([seed_batch + n_sample])
            continue
...
def test_model(X_test,Y_test,M_test,timestamps,feat_sensor_names,label_names,model,fold, n_train, results_per_participant):
    ...
    while count < len(cv5_files):
        ...
        if count % 4 == 0  and modeled == False:
            ...
            for (idx, sensors_to_use) in enumerate(sensors):
                file1 = open("./results/results_single_sensors_OAL_fold_" + str(fold) + "_gamma_" + str(gamma) + "_sensor_" + sensors_to_use + ".csv", "w")
                results_per_participant = csv.writer(file1)
                ...
                for (idx, target_labels) in enumerate(context_labels):
                    ...
                    train_test_model(X_train,y_train,M_train,feat_sensor_names,label_names,sensors_to_use,
                                     target_labels, seed, X_test,y_test,M_test,timestamps_test,fold, results_per_participant)
